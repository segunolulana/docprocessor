...
def _get_important_sentences(sentences, corpus, important_docs):
    ...
    return [{"sentence": sentences_by_corpus[tuple(important_doc["doc"])], "score": important_doc["score"]} for important_doc in important_docs]


def _get_sentences_with_word_count(sentences_full_form, word_count):
    ...
    """
    length = 0
    selected_sentences_full_form = []

    # Loops until the word count is reached.
    for sentence_full_form in sentences_full_form:
        sentence = sentence_full_form["sentence"]
        words_in_sentence = len(sentence.text.split())

        # Checks if the inclusion of the sentence gives a better approximation
        # to the word parameter.
        if abs(word_count - length - words_in_sentence) > abs(word_count - length):
            return selected_sentences_full_form

        selected_sentences_full_form.append({"sentence": sentence, "score": sentence_full_form["score"]})
...
def _extract_important_sentences(sentences, corpus, important_docs, word_count):
    ...
    """
    important_sentences_full_form = _get_important_sentences(sentences, corpus, important_docs)

    # If no "word_count" option is provided, the number of sentences is
    # reduced by the provided ratio. Else, the ratio is ignored.
    return important_sentences_full_form \
        if word_count is None \
        else _get_sentences_with_word_count(important_sentences_full_form, word_count)
...
def _format_results(extracted_sentences, split):
    ...
    """
    if split:
        return [(sentence_full_form["score"], sentence_full_form["sentence"]) for sentence_full_form in extracted_sentences]
    return "\n".join(sentence_full_form["sentence"].text for sentence_full_form in extracted_sentences)
...
def summarize_corpus(corpus, ratio=0.2):
    ...
    # logger.debug(ic.format(hashable_corpus))
    # logger.debug(ic.format(pagerank_scores))

    logger.info('Sorting pagerank scores')
    hashable_corpus.sort(key=lambda doc: pagerank_scores.get(doc, 0), reverse=True)
    result = [{"doc": list(doc), "score": pagerank_scores.get(doc, 0)} for doc in hashable_corpus[:int(len(corpus) * ratio)]]
    logger.debug(ic.format(result))
    return result


def summarize(text, ratio=0.2, word_count=None, split=False, language="english", additional_stopwords=None):
    ...
    logger.debug(ic.format(extracted_sentences))

    # Sorts the extracted sentences by apparition order in the original text.
    extracted_sentences.sort(key=lambda s: s["sentence"].index)
    result = _format_results(extracted_sentences, split)
    logger.debug(ic.format(result))
    return result
